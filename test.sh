...
mkdir -p /tmp/push_swap
rm /tmp/push_swap/*.cnt
python3 -c "from itertools import permutations as p ; print('\n'.join([ ' '.join(x) for x in p('123', 3)]))" | while read -r line; do
	echo -n '`'$line'`: '
	# ./push_swap $line
	operation=$(./push_swap $line)
	ops_count=$(echo  "$operation" | wc -l)
...
# mkdir -p /tmp/push_swap
# rm /tmp/push_swap/*.cnt
# python3 -c "from itertools import permutations as p ; print('\n'.join([ ' '.join(x) for x in p('123456', 6)]))" | while read -r line; do
# 	echo -n '`'$line'`: '
# 	# ./push_swap $line
# 	operation=$(./push_swap $line)
# 	ops_count=$(echo  "$operation" | wc -l)
...
# mkdir -p /tmp/push_swap
# rm /tmp/push_swap/*.cnt
# python3 -c "from itertools import permutations as p ; print('\n'.join([ ' '.join(x) for x in p('12345678', 8)]))" | while read -r line; do
# 	echo -n '`'$line'`: '
# 	# ./push_swap $line
# 	operation=$(./push_swap $line)
# 	ops_count=$(echo  "$operation" | wc -l)
...
# test_ps "2 1"
# test_ps "1 2 0"
# test_ps "234 432 345 543 456 654 567 765 678 789"
